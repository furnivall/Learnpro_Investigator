...
def take_in_dir(list_of_modules):
    ...
    print(str(lp_count + non_lp_count) + " files read. " + str(lp_count) + " contained learnpro data, " + str(
        non_lp_count) +
          " did not.")
    df_users = master['Email / Username'].unique().tolist()
...
named_list_df = named_list_df.assign(email = learnpro_users)
named_list_df.set_index("email", inplace=True)
for module in module_dfs:
    mod = module_dfs.get(module)

    named_list_df[module] = np.where(named_list_df.index.isin(mod['Email / Username']), 1, 0)
    print(mod.columns)

named_list_df.index = named_list_df.index.str.lower()
...
named_list_df.to_excel('C:/Learnpro_Extracts/heather-final.xlsx')
